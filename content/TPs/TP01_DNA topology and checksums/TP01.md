...
Biological sequence data files are stored in databases with a unique number or **identifier** (also sometimes called "key") that identifies the sequence like a key in a dictionary. This identifier is essential for any database to keep sequences organized.

For [GenBank](http://www.ncbi.nlm.nih.gov/genbank/), the identifier is called **ACCESSION** number (highlighted in yellow below).

![[TPs/TP01_DNA topology and checksums/TP01_001.png]]

Since there are several overlapping databases, the **same** sequence is often stored in different databases under **different** keys that are specific to each database. This is a problem since reference tables with the different identifiers have to be created and maintained in order to identify the same sequence in different databases.

The **se**quence **g**lobally **u**nique **id**entifier ([**seguid**](http://onlinelibrary.wiley.com/doi/10.1002/pmic.200600032/abstract;jsessionid=8E73EB1CB7522FF9D3CD411A848CDF5B.f01t03)) was proposed to solve this problem by allowing the calculation of a unique **27 character** code from the sequence itself. The calculation of the **seguid** is based on a cryptographic checksum of the sequence data and is for this reason the same in any database as it only depends on the sequence itself.

The seguid checksum has been extendedto DNA  (**SEGUID v2**) , which is a more complex case, since DNA can be linear or circular as well as single or double stranded.
...
|              | ==s==ingle strand | ==d==ouble strand |
| ------------ | ----------------- | ----------------- |
| ==l==inear   | `lsseguid`        | `ldseguid`        |
| ==c==ircular | `csseguid`        | `cdseguid`        |

## An example

The [pUC19](https://en.wikipedia.org/wiki/PUC19) is a commonly used _E. coli_ cloning vector. It is a **circular double stranded DNA** molecule with 2686 base pairs (bp).

![[TPs/TP01_DNA topology and checksums/image-1.png]]

Open the pUC19 sequence in raw format [here](<TPs/TP01_DNA topology and checksums/plasmid_pUC19_in_raw_sequence_format.txt>). The sequence can also be found in GenBank under the accession number [L09137](http://www.ncbi.nlm.nih.gov/nuccore/L09137). The SEGUID calculator can be accessed [here](https://seguid.github.io/seguid-calculator). Paste the pUC19 sequence in raw format (just the A, G, C and Ts) into the large window as in the image below.

![[TPs/TP01_DNA topology and checksums/TP01_Calculating_checksums_using_seguid_calculator-1.png]]
...
![[TPs/TP01_DNA topology and checksums/TP01_Calculating_checksums_using_seguid_calculator-2.png]]

The new checksum is **`cdseguid=Dz1EGySYXJfjSj9-EMxpsGC7NR4`**. As we can see, the new result is **very** different although only one out of a total of 2686 nucleotides was changed.

> [!NOTE]
> Identical sequences in [UPPERCASE](https://pt.wikipedia.org/wiki/Caixa_alta_e_caixa_baixa) or lowercase or mixed case have the same seguid checksum.
...
## _**The clseguid and cdseguid checksums**_
...
| Sequence                                                                                                   |
|:---------------------------------------------------------------------------------------------------------- |
| [JD009973](<TPs/TP01_DNA topology and checksums/JD009973_in_raw_sequence_format.txt>)   |
| [L09137](<TPs/TP01_DNA topology and checksums/L09137_in_raw_sequence_format.txt>)       |
| [M77789](<TPs/TP01_DNA topology and checksums/M77789_in_raw_sequence_format.txt>)       |
| [pUC19](<TPs/TP01_DNA topology and checksums/plasmid_pUC19_in_raw_sequence_format.txt>) |
| [JD007526](<TPs/TP01_DNA topology and checksums/JD007526_in_raw_sequence_format 1.txt>)                                                                                                |
...
Please answer as as indicated for the first example student "_Max Maximus_".
...
- Bassi, S., Bassi, S., and Gonzalez, V. (2007) New checksum functions for Biopython. _Nature Precedings_. [Link](http://precedings.nature.com/documents/278/version/1). This presentation deals with SEGUID implementation.
