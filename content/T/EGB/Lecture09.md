### Lecture09 - Protein production and genetic engineering in Agriculture

This lecture addresses the production of proteins from cloned genes and the application of gene cloning and DNA analysis in agriculture. It covers heterologous expression systems, expression vectors, promoters, fusion tags, host strains, expression optimization, protein purification, functional verification, and agricultural biotechnology applications, including transgenic technologies and molecular markers.

#### Key Concepts

- Protein production from cloned genes
- Heterologous gene expression
- Bacterial, yeast, and eukaryotic expression systems
- Expression vectors
- Promoters
- Fusion tags
- Host strain selection
- Protein yield, solubility, and stability
- Codon optimization
- Induction conditions
- Subcellular targeting
- Protein purification
- Functional verification
- Genes underlying agricultural traits
- Genetically modified organisms
- Molecular markers
- DNA fingerprinting
- Transgenic technologies
- Crop and livestock improvement
- Ethical, environmental, and regulatory considerations

#### Literature

Chapter 14 Production of Protein from Cloned Genes (optional lecture [video](https://youtu.be/bRFy1w1j86E?si=HvDYUUjjfVI_fCOK))
Chapter 16 Gene Cloning and DNA Analysis in Agriculture (optional lecture [video](https://youtu.be/uZcxaex2Esc?si=xaKJJ4NfGzFaNE4L))
